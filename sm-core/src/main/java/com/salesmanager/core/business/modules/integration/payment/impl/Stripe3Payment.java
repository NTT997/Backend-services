...
//			strAmount = strAmount.replace(".", "");
			...
			String strAmount = productPriceUtils.getAmountInMinorUnits(store, amount); //Tho
			
			PaymentIntentCreateParams createParams = new PaymentIntentCreateParams.Builder()
					.setCurrency(store.getCurrency().getCode()).setAmount(Long.parseLong(strAmount))
	...
	public Transaction authorize(MerchantStore store, Customer customer, List<ShoppingCartItem> items,
		...
		try {
			...
			transaction.getTransactionDetails().put("TRNORDERNUMBER", paymentIntent.getId()); // <---- We store the PI
																								// id here
			transaction.getTransactionDetails().put("MESSAGETEXT", null);
	...
	public Transaction capture(MerchantStore store, Customer customer, Order order, Transaction capturableTransaction,
		...
		try {
			...
			String chargeId = capturableTransaction.getTransactionDetails().get("TRNORDERNUMBER"); // <---- We retrieve
																									// the PI id here
...
//			strAmount = strAmount.replace(".", "");
			...
			String strAmount = productPriceUtils.getAmountInMinorUnits(store, order.getTotal()); //Tho
			...
			PaymentIntentCaptureParams params = PaymentIntentCaptureParams.builder()
					...
					.setStatementDescriptor(store.getStorename().length() > 22 ? store.getStorename().substring(0, 22)
							: store.getStorename())
					.build();
...
//			strAmount = strAmount.replace(".", "");
			 ...
			 * chargeParams.put("amount", strAmount); chargeParams.put("capture", true);
			 * chargeParams.put("currency", store.getCurrency().getCode());
			 * chargeParams.put("source", token); // obtained with Stripe.js
			 * chargeParams.put("description", new
			 * StringBuilder().append(TRANSACTION).append(" - ").append(store.getStorename()
			 * ).toString());
			 */
