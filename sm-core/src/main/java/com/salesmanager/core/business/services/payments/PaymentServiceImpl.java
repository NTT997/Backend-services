	...
	public Map<String, IntegrationConfiguration> getPaymentModulesConfigured(MerchantStore store)
		...
		try {
			...
			MerchantConfiguration merchantConfiguration = merchantConfigurationService
					.getMerchantConfiguration(Constants.PAYMENT_MODULES, store);
...
//					modules = ConfigurationModulesLoader.loadIntegrationConfigurations(decrypted);
					modules = ConfigurationModulesLoader
							.loadIntegrationConfigurations(merchantConfiguration.getValue());
	...
	@Override
	public Transaction processRefund(Order order, Customer customer, MerchantStore store, BigDecimal amount, List<OrderProduct> listOrderProduct)
			throws ServiceException {
		...
		if(partial == false) {
			...
			OrderTotal partialRefund = new OrderTotal();
			partialRefund.setModule(Constants.OT_REFUND_MODULE_CODE);
			partialRefund.setText(Constants.OT_REFUND_MODULE_CODE);
			partialRefund.setTitle(Constants.OT_REFUND_MODULE_CODE);
			partialRefund.setSortOrder(100);
			partialRefund.setOrderTotalType(OrderTotalType.REFUND);
			partialRefund.setValue(amount);
			partialRefund.setOrderTotalCode(Constants.OT_REFUND_MODULE_CODE);
			
			partialRefund.setOrder(order);
			
			order.getOrderTotal().add(partialRefund);
			...
			OrderStatusHistory orderHistory = new OrderStatusHistory();
			orderHistory.setOrder(order);
			orderHistory.setStatus(OrderStatus.PARTIALLY_REFUNDED);
			orderHistory.setDateAdded(new Date());
			order.getOrderHistory().add(orderHistory);			
	...
	public void validateCreditCard(String number, CreditCardType creditCard, String month, String date)
		...
		try {
			...
			throw new ServiceException(ServiceException.EXCEPTION_VALIDATION, "Invalid date format",
					"messages.error.creditcard.dateformat");
		}

		if (StringUtils.isBlank(number)) {
			throw new ServiceException(ServiceException.EXCEPTION_VALIDATION, "Invalid card number",
	...
	@Override
	public Transaction initTransaction(Order order, Customer customer, Payment payment, MerchantStore store)
			throws ServiceException {
...
//	}
	...
	public Transaction initTransaction(Customer customer, Payment payment, MerchantStore store)
		...
		try {
			...
			Validate.notNull(payment.getAmount(), "Payment amount must not be null");

			LOGGER.debug("Initializing payment transaction for store: {}, payment: {}, customer: {}", store.getCode(),
					payment.getModuleName(), customer != null ? customer.getId() : "null");
			...
			Map<String, IntegrationConfiguration> modules = this.getPaymentModulesConfigured(store);
			if (modules == null) {
				LOGGER.error("No payment modules configured for store {}", store.getCode());
				throw new ServiceException("No payment module configured for store " + store.getCode());
			}

			String moduleName = payment.getModuleName();
			if (moduleName == null) {
				LOGGER.error("Payment module name is null");
				throw new ServiceException("Payment module name is required");
			}

			IntegrationConfiguration configuration = modules.get(moduleName);
			if (configuration == null) {
				LOGGER.error("No configuration found for payment module {}", moduleName);
				throw new ServiceException("Payment module " + moduleName + " is not configured");
			}

			if (!configuration.isActive()) {
				LOGGER.warn("Payment module {} is not active", moduleName);
				throw new ServiceException("Payment module " + moduleName + " is not active");
			}

			PaymentModule module = this.paymentModules.get(moduleName);
			if (module == null) {
				LOGGER.error("No implementation found for payment module {}", moduleName);
				throw new ServiceException("Payment module " + moduleName + " does not exist");
			}

			IntegrationModule integrationModule = getPaymentMethodByCode(store, moduleName);

			Transaction transaction = module.initTransaction(store, customer, amount, payment, configuration,
					integrationModule);
			...
			throw new ServiceException("Failed to initialize payment transaction: " + e.getMessage(), e);
		}
	}
