...
public class PayPalExpressCheckoutPayment implements PaymentModule {
	...
	@Inject
	private ProductPriceUtils productPriceUtils; // Tho
	...
	/*
	 * @Override public Transaction capture(MerchantStore store, Customer customer,
	 * List<ShoppingCartItem> items, BigDecimal amount, Payment payment, Transaction
	 * transaction, IntegrationConfiguration configuration, IntegrationModule
	 * module) throws IntegrationException {
	 * 
	...
	public Transaction initPaypalTransaction(MerchantStore store, List<ShoppingCartItem> items,
			OrderTotalSummary summary, Payment payment, IntegrationConfiguration configuration,
...
//				amt.setValue(pricingService.getStringAmount(cartItem.getFinalPrice().getFinalPrice(), store));
				...
				String strAmount = productPriceUtils.getAmountInMajorUnits(store,
						cartItem.getFinalPrice().getFinalPrice()); // Tho
				amt.setValue(strAmount);
...
//					shipping.setValue(pricingService.getStringAmount(total.getValue(), store));
					...
					String strAmount = productPriceUtils.getAmountInMajorUnits(store, total.getValue()); // Tho
					shipping.setValue(strAmount);
...
//				taxAmnt.setValue(pricingService.getStringAmount(tax, store));
				...
				String strAmount = productPriceUtils.getAmountInMajorUnits(store, tax); // Tho
				taxAmnt.setValue(strAmount);
...
//			itemTotal.setValue(pricingService.getStringAmount(summary.getSubTotal(), store));
			...
			String strAmountItemTotal = productPriceUtils.getAmountInMajorUnits(store, summary.getSubTotal()); // Tho
			itemTotal.setValue(strAmountItemTotal);
...
//			orderTotal.setValue(pricingService.getStringAmount(summary.getTotal(), store));
			...
			String strAmountOrderTotal = productPriceUtils.getAmountInMajorUnits(store, summary.getSubTotal()); // Tho
			orderTotal.setValue(strAmountOrderTotal);
			...
			if (!StringUtils.isBlank(scheme)) {
				baseScheme = new StringBuilder().append(coreConfiguration.getProperty("SHOP_SCHEME", "http"))
						.append("://").append(store.getDomainName()).toString();
			}
...
//			 refundAmount.setValue(pricingService.getStringAmount(amount, store));
			...
			String strRefundAmount = productPriceUtils.getAmountInMajorUnits(store, amount); // Tho

			refundAmount.setValue(strRefundAmount);
...
//			 orderTotal.setValue(pricingService.getStringAmount(amount, store));
			...
			String strAmount = productPriceUtils.getAmountInMajorUnits(store, amount); // Tho
			orderTotal.setValue(strAmount);
			orderTotal.setCurrencyID(
					urn.ebay.apis.eBLBaseComponents.CurrencyCodeType.fromValue(payment.getCurrency().getCode()));
...
//				 amount.setValue(pricingService.getStringAmount(order.getTotal(), store));
			...
			String strAmount = productPriceUtils.getAmountInMajorUnits(store, order.getTotal()); // Tho
			amount.setValue(strAmount);
