...
import com.salesmanager.shop.store.security.NewPasswordRequest;
...
import com.salesmanager.shop.store.security.ResetPasswordToken;
...
//	}
	...
	@ResponseStatus(HttpStatus.OK)
	@PostMapping(value = {
			"/customer/reset/verify" }, consumes = MediaType.APPLICATION_JSON_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
	@ApiOperation(httpMethod = "POST", value = "Validate customer password reset token", notes = "", response = Void.class)
	@ApiImplicitParams(@ApiImplicitParam(name = "lang", dataType = "String", defaultValue = "en"))
	public void passwordResetVerify(@RequestBody ResetPasswordToken request, @ApiIgnore MerchantStore merchantStore,
		...
		customerFacade.verifyPasswordRequestToken(request.getToken(), request.getStore());
...
//	}
	...
	@PostMapping(value = "/customer/password/new-password", consumes = MediaType.APPLICATION_JSON_VALUE, produces = {
			"application/json" })
	@ApiOperation(httpMethod = "POST", value = "Change customer password", response = Void.class)
	public void changePassword(@RequestBody @Valid NewPasswordRequest newPasswordRequest,
			@ApiIgnore MerchantStore merchantStore, @ApiIgnore Language language, HttpServletRequest request) {

		// validate password
		if (StringUtils.isBlank(newPasswordRequest.getPassword())
				|| StringUtils.isBlank(newPasswordRequest.getRepeatPassword())) {
			throw new RestApiException("400", "Password don't match");
		}

		if (!newPasswordRequest.getPassword().equals(newPasswordRequest.getRepeatPassword())) {
			throw new RestApiException("400", "Password don't match");
		}

		customerFacade.resetPassword(newPasswordRequest.getPassword(), newPasswordRequest.getToken(),
				newPasswordRequest.getStore());
