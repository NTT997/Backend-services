...
//	}
	...
	public CustomerReview populate(PersistableCustomerReview source, CustomerReview target, MerchantStore store,
		...
		try {
			...
			CustomerReviewDescription existingDescription = null;
			if (target.getDescriptions() != null) {
				for (CustomerReviewDescription d : target.getDescriptions()) {
					if (d.getLanguage().getCode().equals(lang.getCode())) {
						existingDescription = d;
						break;
					}
				}
			} else {
				target.setDescriptions(new HashSet<>());
			}

			if (existingDescription == null) {
				existingDescription = new CustomerReviewDescription();
				existingDescription.setLanguage(lang);
				existingDescription.setCustomerReview(target);
				existingDescription.setName("-");
				target.getDescriptions().add(existingDescription);
			}
